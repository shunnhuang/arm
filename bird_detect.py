...
current_pan  = 90   # 当前水平角度
current_tilt = 90   # 当前垂直角度
 
PAN_DEADZONE  = 3   # 水平死区 (度)
TILT_DEADZONE = 3   # 垂直死区 (度)
 
last_servo_time  = 0
SERVO_INTERVAL   = 0.05   # 最快 50ms 发一次指令，防止串口堵塞
...
while True:
    ...
    for box in boxes:
        ...
        # SERVO: 画面坐标 → 舵机角度
        #
        # 水平 (pan):  center_x 在 [0, WIDTH]  → [0°, 180°]
        # 垂直 (tilt): center_y 在 [0, HEIGHT] → [0°, 180°]
        #              图像Y轴向下，舵机向上为小角度，取反
        # =====================================
        ...
        label = f"PAN:{target_pan}  TILT:{target_tilt}"
